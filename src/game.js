...
function drawBoard() {
  ...
  for (let i = 0; i < 9; i++) {
    ADJACENCY[i].forEach(j => {
      if (j <= i) return;
      const a = cellToXY(i), b = cellToXY(j);
      // Les diagonales sont légèrement plus fines
      const isDiag = (Math.abs(a.x - b.x) === CELL_SIZE && Math.abs(a.y - b.y) === CELL_SIZE);
  ...
  for (let i = 0; i < 9; i++) {
    ...
    if (state.selected === i) {
      ...
      ctx.fillStyle = 'rgba(232,168,56,.2)'; ctx.fill();
    }
    ...
    if (state.selected !== -1 && state.phase === 2 && cell === EMPTY) {
      ...
      if (legalDests.includes(i)) {
        ...
        ctx.fillStyle = 'rgba(232,168,56,.6)'; ctx.fill();
        ctx.strokeStyle = '#e8a838'; ctx.lineWidth = 1.5; ctx.stroke();
      }
    }
    ...
    if (cell !== EMPTY) {
      ...
      // Ombre
      ctx.beginPath(); ctx.arc(x+3, y+4, 22, 0, Math.PI*2);
      ctx.fillStyle = 'rgba(0,0,0,.45)'; ctx.fill();

      // Corps du pion (dégradé radial)
      const g = ctx.createRadialGradient(x-6, y-6, 2, x, y, 22);
      if (cell === P1) {
        g.addColorStop(0, '#666'); g.addColorStop(1, '#0d0d0d');
      } else {
        g.addColorStop(0, '#ffffff'); g.addColorStop(1, '#c8c0b0');
      }
...
function applyMove(st, move) {
  ...
  return { ...st, board: nb, phase, placed, turn: st.turn === P1 ? P2 : P1, selected: -1 };
}
...
function evaluate(board, player, phase) {
  ...
  for (const [a,b,c] of WIN_LINES) {
    ...
    if (his === 0) {
      if (mine === 2 && free === 1) score += 100;  // menace directe
      else if (mine === 1 && free === 2) score += 10;
    }
    if (mine === 0) {
      if (his === 2 && free === 1) score -= 100;   // bloquer adversaire
  ...
  if (phase === 2) {
    let myMoves = 0, oppMoves = 0;
    for (let from = 0; from < 9; from++) {
      const adj = ADJACENCY[from];
      if (board[from] === player) myMoves  += adj.filter(t => board[t] === EMPTY).length;
      if (board[from] === opp)    oppMoves += adj.filter(t => board[t] === EMPTY).length;
    }
...
let _nodes = 0, _cutoffs = 0, _ttHits = 0;
...
function minimax(st, depth, alpha, beta, maximizing, aiPlayer) {
  ...
  if (entry && entry.depth >= depth) {
    ...
    if (alpha >= beta) { _cutoffs++; return entry.value; }
  }
  ...
  for (const move of moves) {
    ...
    if (maximizing) {
      if (val > best) { best = val; flag = TT_EXACT; }
    ...
    if (alpha >= beta) { _cutoffs++; break; }  // Élagage Alpha-Beta
  }

  // Stocker le résultat dans la TT
  const storedFlag = maximizing
    ? (best <= origAlpha ? TT_UPPER : (best >= beta  ? TT_LOWER : TT_EXACT))
    : (best >= origBeta  ? TT_LOWER : (best <= alpha ? TT_UPPER : TT_EXACT));
...
function getBestMove(st, difficulty) {
  ...
  const timeLimit = difficulty === 'hard' ? 1500 : difficulty === 'medium' ? 600 : 200; // ms

  _nodes = 0; _cutoffs = 0; _ttHits = 0;
  _tt.clear();
  ...
  for (let depth = 1; depth <= maxDepth; depth++) {
    ...
    for (const move of orderedMoves) {
      if (performance.now() - t0 > timeLimit) break;
      const child = applyMove(st, move);
      const val   = minimax(child, depth - 1, -Infinity, Infinity, false, st.turn);
      if (val > iterBest) { iterBest = val; iterMove = move; }
    }

    if (iterMove) { bestMove = iterMove; depthReached = depth; }
...
function onCellClick(idx) {
  ...
  if (state.phase === 1) {
    ...
    if (state.selected === -1) {
      if (state.board[idx] === state.turn) { state.selected = idx; drawBoard(); }
    } else {
      if (state.board[idx] === state.turn) {
        state.selected = idx; drawBoard();
      } else if (state.board[idx] === EMPTY && ADJACENCY[state.selected].includes(idx)) {
        doMove({ type: 'move', from: state.selected, to: idx });
      } else {
        state.selected = -1; drawBoard();
      }
    }
  }
}
...
function doMove(move) {
  ...
  // Tour de l'IA ?
  const isAITurn = (state.mode === 'hva' && state.turn === P2)
                || (state.mode === 'ava');
...
function updateStatus() {
  ...
  document.getElementById('p1-card').classList.toggle('active', state.turn === P1);
  document.getElementById('p2-card').classList.toggle('active', state.turn === P2);
...
function updateAIStats() {
  ...
  document.getElementById('ai-nodes').textContent  = s.nodesVisited.toLocaleString();
...
function showEndModal(winner) {
  ...
  document.getElementById('modal-icon').textContent  = winner ? (winner===P1?'🥇':'🏆') : '🤝';
  document.getElementById('modal-title').textContent = winner ? `${names[winner]} gagne !` : 'Match nul';
...
function startGame(mode, difficulty = 'medium') {
  state = {
    ...
    gameOver: false,
    aiThinking: false,
    aiStats: { timeMs:0, nodesVisited:0, depthReached:0, ttHits:0, cutoffs:0 },
    tt: new Map(),
  };

  const diffNames = { easy:'Facile', medium:'Moyen', hard:'Difficile' };
  document.getElementById('p1-name').textContent = mode === 'ava' ? 'IA Noire' : 'Joueur 1';
  document.getElementById('p2-name').textContent = mode === 'hvh' ? 'Joueur 2' : `IA (${diffNames[difficulty]})`;
  document.getElementById('game-title').textContent =
    { hvh:'Humain vs Humain', hva:'Humain vs IA', ava:'IA vs IA — Démo' }[mode];
